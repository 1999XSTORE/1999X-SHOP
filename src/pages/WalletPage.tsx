...
import { Wallet, ArrowRight, ArrowLeft, RefreshCw, Users, Check, X, Copy, CheckCircle, ExternalLink, Loader2 } from 'lucide-react';
...
function AdminPanel() {
  ...
  useEffect(() => {
    loadTxns();
    // Auto-refresh every 15 seconds so admin sees new submissions
    intervalRef.current = setInterval(loadTxns, 15000);
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-2">
        {loading ? (
          ...
          <div className="text-center py-10 text-white/30 text-sm">No {filter} transactions</div>
        ) : (
          filtered.map(tx => (
            <div
              key={tx.id}
...
function PaymentCard({ method, amount }: { method: typeof PAYMENT_METHODS[number]; amount: number }) {
  ...
  if (!method.hasQr) {
    // PayPal — no QR, just branded button
    return (
      <div className={cn('rounded-2xl border p-6 text-center space-y-5', method.bgClass)}>
        ...
        <a
          href={method.fields[0].value}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center gap-2 w-full py-4 rounded-xl font-bold text-white text-sm transition-all active:scale-[0.97]"
      ...
      <div className="p-5 space-y-4">
        ...
        {method.qr && !method.qr.startsWith('YOUR_') && (
          <div className="flex justify-center">
            <div className="bg-white rounded-2xl p-3 shadow-lg shadow-black/30" style={{ width: 180, height: 180 }}>
              <img
                src={method.qr}
                alt={`${method.label} QR`}
          ...
          <div className="flex justify-center">
            <div className="bg-white/5 border border-dashed border-white/20 rounded-2xl flex items-center justify-center" style={{ width: 180, height: 180 }}>
              <div className="text-center">
                ...
                <p className="text-[10px] text-white/30">QR code<br/>coming soon</p>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              {!field.value.startsWith('YOUR_') && (
                <button
                  ...
                  className="flex-shrink-0 p-2 rounded-lg bg-white/5 hover:bg-white/10 transition-colors active:scale-[0.95]"
                >
          ...
          <div key={i} className="flex items-center gap-2 flex-1">
            <div className="flex items-center gap-2 flex-1">
              <div className={cn(
                ...
                active ? 'bg-purple-600 text-white ring-2 ring-purple-500/40' :
                         'bg-white/5 text-white/30'
              )}>
                {done ? <CheckCircle className="w-4 h-4" /> : n}
              </div>
              <span className={cn('text-xs font-semibold transition-all', active ? 'text-white' : done ? 'text-emerald-400' : 'text-white/30')}>
                {label}
              </span>
            </div>
            {i < steps.length - 1 && (
              <div className={cn('h-px flex-1 transition-all', done ? 'bg-emerald-500/50' : 'bg-white/10')} />
            )}
          </div>
        );
      })}
...
export default function WalletPage() {
  ...
  useEffect(() => {
    ...
    // Realtime — auto-credit balance when admin approves
    const channel = supabase
        ...
        <div className="rounded-2xl p-5 border border-white/10 bg-white/3">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              ...
              <p className="text-xs text-white/30">Auto-refreshes every 15 seconds · realtime updates enabled</p>
            </div>
          </div>
          <AdminPanel />
        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                {AMOUNTS.map(a => (
                  <button
                    ...
                    className={cn(
                      'py-3.5 rounded-xl text-sm font-bold transition-all active:scale-[0.97] border',
                      amount === a && !customAmount
                        ? 'bg-purple-500/20 text-purple-300 border-purple-500/40 shadow-lg shadow-purple-500/10'
                        : 'bg-white/5 text-white/60 hover:bg-white/8 border-white/5'
                    )}
                  >
                    ${a}
                  </button>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-4 py-3.5 rounded-xl bg-white/5 border border-white/10 text-sm text-white placeholder:text-white/20 focus:outline-none focus:ring-2 focus:ring-purple-500/40 focus:border-purple-500/40"
                />
              </div>
              {selectedAmount > 0 && (
              ...
              <button
                onClick={() => { if (selectedAmount > 0) setStep(2); else toast.error('Select an amount'); }}
                className="w-full flex items-center justify-center gap-2 py-4 rounded-xl font-bold text-sm text-white transition-all active:scale-[0.97]"
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none -mx-1 px-1">
                {PAYMENT_METHODS.map(m => (
                  <button
                    ...
                    className={cn(
                      'flex items-center gap-2 px-4 py-2.5 rounded-full text-xs font-bold whitespace-nowrap flex-shrink-0 border transition-all',
                      methodId === m.id
              ...
              <div className="flex gap-2 pt-1">
                <button
                  onClick={() => setStep(1)}
                  className="flex items-center justify-center gap-1.5 px-5 py-3.5 rounded-xl bg-white/5 text-white/60 text-sm font-semibold hover:bg-white/8 transition-all border border-white/5"
                >
                  <ArrowLeft className="w-4 h-4" /> Back
                </button>
                <button
                  onClick={() => setStep(3)}
                  className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl font-bold text-sm text-white transition-all active:scale-[0.97]"
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3.5 rounded-xl bg-white/5 border border-white/10 text-sm text-white placeholder:text-white/20 focus:outline-none focus:ring-2 focus:ring-purple-500/40 focus:border-purple-500/40"
                  autoComplete="off"
                />
                <p className="text-[10px] text-white/30 mt-2">
                  Enter the transaction ID, reference number, or payment confirmation code you received
                </p>
              </div>
